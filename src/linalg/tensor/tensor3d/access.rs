    ...
    > Tensor3D<CHANNELS, ROWS, COLS, S>
{
    pub fn get(self: &Self, c: usize, i: usize, j: usize) -> Scalar {
        debug_assert!(c < CHANNELS && i < ROWS && j < COLS);
        let flat_index: usize = c * (ROWS * COLS) + i * COLS + j;
    ...
    /// # Safety
    /// The caller guarantees c < CHANNELS, i < ROWS, j < COLS.
    ...
    /// # Safety
    /// The caller guarantees c < CHANNELS, i < ROWS, j < COLS.
    pub unsafe fn set_unchecked(self: &mut Self, c: usize, i: usize, j: usize, value: Scalar) -> ()
    where
        S: StorageMut<[[[Scalar; COLS]; ROWS]; CHANNELS]>,
    {
        let flat_index: usize = c * (ROWS * COLS) + i * COLS + j;
    ...
    /// # Safety
    /// The caller guarantees c < CHANNELS, i < ROWS.
